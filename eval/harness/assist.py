...
ANSWERS = REPO / "eval" / "answers" / "claimable.json"
...
def run(client: Any, *, batch_size: int, limit: int | None) -> Arm:
    from neti.insight.assist import SYSTEM, batches, eligible, parse, question, schema
...
@dataclass
class Unclaimed:
    """Arm C. Of the parameters no rule touches, which does a model claim — and is it right?

    Arms A and B ask what a model does where somebody already decided. This asks the question the
    feature exists for, and it is the only arm whose answer key is an opinion: `eval/answers/
    claimable.json`, generated by a script whose rules are written out and reviewable.

    The fields are split the way an operator reads them. `found` is the upside — a set the rule
    table missed and a shipped resolver could size. `false_claim` is the cost — a resolver bound to
    something that is not a set, which resolves to UNRESOLVED on every call and is discovered
    afterwards. `unadjudicated` is neither, and is excluded from every rate.
    """

    of: int = 0
    """Adjudicated pairs only. The unadjudicated ones are not in this denominator."""

    found: int = 0
    """Key names a resolver; the model named the same one."""

    found_wrong_resolver: int = 0
    """Key names a resolver; the model named a different one."""

    found_missed: int = 0
    """Key names a resolver; the model saw nothing."""

    false_claim: int = 0
    """Key says not_a_set; the model named a resolver anyway. The number that decides safety."""

    forced_near_miss: int = 0
    """Key says no shipped resolver could size it; the model named one regardless.

    Separated from `false_claim` because it is a different mistake with a different fix. The
    parameter really does name a set — the model was right about that and wrong about the tool —
    and the answer `no_shipped_resolver` exists precisely so it does not have to guess.
    """

    agreed_no_resolver: int = 0
    """Key says no shipped resolver could size it, and the model said so too.

    Worth counting on its own: these are `RESOLVER_CONTRACT.md`'s waiting list, arrived at
    independently by a model and by a person.
    """

    quiet: int = 0
    """Adjudicated not_a_set, and the model agreed. The vast majority, and the boring one."""
    ...
    @property
    def wrong(self) -> int:
        return self.false_claim + self.forced_near_miss + self.found_wrong_resolver


def _specs_for_unclaimed(decisions: dict[str, dict[str, Any]]) -> list[Any]:
    """Every tool with a parameter no rule claims — exactly what `neti suggest` sends.

    Unlike arms A and B this needs no reconstruction: `eligible()` already selects precisely this
    set, so the harness and the shipped command are asking the same question of the same tools.
    """
    from neti.insight.discover import DeclinedParam, ToolSpec

    out = []
    for key, entry in sorted(decisions.items()):
        unclaimed = [d for d in entry["declined"] if d.get("would_be") is None]
        if not unclaimed:
            continue
        params = sorted(
            {g["pointer"].lstrip("/").split("#")[0] for g in entry["gated"]}
        ...
        out.append(
            ToolSpec(
                ...
                gated=(),
                destructive=entry.get("destructive", False),
                declined=tuple(DeclinedParam(param=d["param"], why="unclaimed") for d in unclaimed),
            )
        )
    return out


def _answer_key() -> dict[tuple[str, str], str]:
    if not ANSWERS.exists():
        raise SystemExit(
            f"error: {ANSWERS.relative_to(REPO)} is missing.\n"
            "  Run `python -m eval.answers.adjudicate` to generate it."
        )
    labels = json.loads(ANSWERS.read_text(encoding="utf-8"))["labels"]
    return {(t, p): e["label"] for t, ps in labels.items() for p, e in ps.items()}


def run_unclaimed(client: Any, *, batch_size: int) -> Unclaimed:
    """Arm C. The 401, against a written adjudication."""
    ...
    decisions = _corpus()
    key = _answer_key()
    specs = _specs_for_unclaimed(decisions)
    candidates = eligible(specs)
    ...
    for index, group in enumerate(groups, start=1):
        ...
        try:
            ...
            print(f"    failed, continuing: {str(exc)[:90]}", file=sys.stderr)
            continue
        asked.update((c.tool, c.parameter) for c in group)
        got, _ = parse(answer.text, group)
        for suggestion in got:
            claimed[(suggestion.tool, suggestion.parameter)] = suggestion.resolver

    for pair in sorted(asked):
        truth = key.get(pair)
        if truth is None or truth == "unadjudicated":
            arm.unadjudicated += 1
            continue
        arm.of += 1
        said = claimed.get(pair, NOT_A_SET)
        where = f"{pair[0]}/{pair[1]}"

        if truth == NOT_A_SET:
            if said in {NOT_A_SET, NO_SHIPPED_RESOLVER}:
                arm.quiet += 1
            else:
                arm.false_claim += 1
                arm.detail.append({"false_claim": where, "model_said": said})
        elif truth == NO_SHIPPED_RESOLVER:
            if said == NO_SHIPPED_RESOLVER:
                arm.agreed_no_resolver += 1
            elif said == NOT_A_SET:
                arm.quiet += 1
            else:
                arm.forced_near_miss += 1
                arm.detail.append({"forced_near_miss": where, "model_said": said})
        else:
            if said == truth:
                arm.found += 1
                arm.detail.append({"found": where, "resolver": said})
            elif said in {NOT_A_SET, NO_SHIPPED_RESOLVER}:
                arm.found_missed += 1
            else:
                arm.found_wrong_resolver += 1
                arm.detail.append({"wrong": where, "expected": truth, "got": said})
    return arm
...
def main() -> int:
    ...
    parser.add_argument(
        "--arm",
        default="both",
        choices=["recovery", "over-claim", "unclaimed", "both", "all"],
        help="recovery is the go/no-go, over-claim the risk, unclaimed the question.",
    )
    ...
    recovery = over_claim = unclaimed_arm = None

    if args.arm in {"recovery", "both", "all"}:
    ...
    if args.arm in {"over-claim", "both", "all"}:
    ...
    if args.arm in {"unclaimed", "all"}:
        print("M12 arm C — the parameters no rule claims", file=sys.stderr)
        unclaimed_arm = run_unclaimed(client, batch_size=args.batch_size)
    ...
    if unclaimed_arm is not None:
        result["unclaimed"] = {k: v for k, v in asdict(unclaimed_arm).items() if k != "detail"}
        detail.extend(unclaimed_arm.detail)
    ...
    if unclaimed_arm is not None:
        u = unclaimed_arm
        print(
            f"unclaimed  of {u.of} adjudicated parameters no rule claims, the model made "
            f"{u.false_claim} claim(s) on\n"
            f"           something that is not a set and {u.forced_near_miss} on a set nothing "
            f"ships can size.\n"
            f"           It found {u.found} the rule table missed, agreed with "
            f"{u.agreed_no_resolver} that no resolver exists,\n"
            f"           and was quiet on {u.quiet}. {u.unadjudicated} pair(s) are unadjudicated "
            "and scored nowhere."
        )
