"""The GitHub resolvers, against a recorded API rather than a live token.

The two behaviours that carry weight are the same two everywhere else in this product: a bare owner
must not resolve like a single repository, and a truncated answer must be a floor rather than a
number.
"""
...
def test_an_org_lookup_failure_is_unresolved_not_zero() -> None:
    out = GitHubReposResolver(FakeApi({})).resolve("acme", CTX)

    assert out.state is ResolutionState.UNRESOLVED
    assert out.magnitude is None, "an unreadable org must never resolve to 0 repositories"
