...
|---|---|---|
| **NC-01** | **Cumulative effect across calls.** 4,000 individual sends are 4,000 calls of one recipient each; per-call resolution sees `1` every time. | Mitigated only by *declared session budgets*, never by resolution. Without a declared budget for the tool, this is invisible. **This claim was false where it mattered most until now**: `SessionTally` lived in memory on the `Engine`, and `neti hook` is one process per tool call — so a declared budget could never fire on the integration the product is mostly installed through. Totals persist beside the records now, and `neti start` declares one. |
| **NC-02** | **Correctness of the action.** Deleting the one row that mattered. | Magnitude is the wrong primitive. A cardinality of 1 is always under every ceiling. *Partially* addressed on two axes, both declared. **Provenance**: a call downstream of untrusted input is judged against a tighter ceiling whatever its size. **Sensitivity**: a target matching a declared rule is gated on what it is rather than how much of it there is, so `.env` can be stopped at cardinality 1. Neither reaches an *undeclared* small target in a clean session, and nothing built on counting ever will. |
